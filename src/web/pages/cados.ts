...
export async function renderCados(): Promise<string> {
  ...
  const data = JSON.stringify({
    ...
    hourLabels,
    byHour: c.byHour,
    byHourCentral: c.byHourCentral,
    dowLabels: DOW,
    byDow: c.byDow,
    byDowCentral: c.byDowCentral,
  });
...
<div class="grid">
  ...
  <div class="card"><div class="k">Busiest hour</div><div class="v">${c.count ? peakHour + ":00" : "—"} <span class="dim" style="font-size:20px">UTC</span></div><div class="sub">${c.count ? peakHourC + ":00 Central · " + c.byHour[peakHour] + " awards" : ""}</div></div>
